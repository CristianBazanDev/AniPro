...
if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
    ...
    if (!in_array($file['type'], $allowed_types)) {
        $_SESSION['error_producto'] = 'Tipo de archivo no permitido. Solo se permiten: JPG, PNG, GIF, WEBP';
        header("Location: ../index.php?view=crear_producto");
        exit;
    }
    ...
    if (!is_dir($upload_dir)) {
        if (!mkdir($upload_dir, 0755, true)) {
            $_SESSION['error_producto'] = 'Error: No se pudo crear el directorio para las imágenes. Verifique los permisos.';
            header("Location: ../index.php?view=crear_producto");
            exit;
        }
    }
    ...
    if (!is_writable($upload_dir)) {
        $_SESSION['error_producto'] = 'Error: El directorio de imágenes no tiene permisos de escritura. Contacte al administrador.';
        header("Location: ../index.php?view=crear_producto");
        exit;
    }
    ...
    if (move_uploaded_file($file['tmp_name'], $filepath)) {
        ...
        $_SESSION['error_producto'] = 'Error al subir la imagen. Verifique los permisos del servidor o intente con otra imagen.';
        header("Location: ../index.php?view=crear_producto");
        exit;
    }
    ...
    $error_msg = isset($upload_errors[$error_code]) 
        ? $upload_errors[$error_code] 
        : 'Error desconocido al subir la imagen (código: ' . $error_code . ')';
